...
│   │                                #   EditModeBar (barra de organizar) + EditableBlock (moldura com alça,
│   │                                #   setas, largura e esconder) + dashboard-prefs.ts (ordem/largura/escondidos,
...
- **Personalizar o painel** (set/2026, agora direto na tela — a janela com a lista abstrata saiu): "Personalizar" liga o **modo de organizar** no próprio `/admin/dashboard`, e o resultado é o painel mudando na frente da pessoa. Barra sempre à vista (`EditModeBar`: no computador presa no alto depois da sidebar, no celular no pé da tela — é `fixed`, porque o `<main>` do painel tem overflow e mata o `sticky`) com a frase do que dá para fazer, o selo "Alterações não salvas", **Salvar**, **Cancelar** e **Restaurar padrão** (só mexe no rascunho; guardar mesmo é no Salvar). Cada bloco ganha a moldura `EditableBlock` com **Arrastar** (alça `@dnd-kit`, funciona no toque e pelo teclado — Espaço, setas, Espaço), **Subir**/**Descer** (o caminho de quem não consegue arrastar), **Largura: Inteira | Metade** e **Esconder**/**Mostrar** — todos com texto, 44px de altura. Bloco escondido continua à vista, apagadinho, com o selo "Escondido". A amostra do bloco é o conteúdo de verdade, cortado em 16rem e `inert` (não recebe clique nem foco). Sair da tela com o rascunho pela metade pergunta antes (`useUnsavedGuard`). Fora do modo, o painel é exatamente o de sempre.
- **Largura dos blocos**: "Inteira" ocupa a linha; dois "Metade" seguidos dividem a linha (no celular tudo é inteira, e "Metade" sem vizinho de meia largura também ocupa a linha toda). Padrão de fábrica: ações, números, cotações, financeiro e agenda inteiros; cursos, cadastros incompletos e últimas ações pela metade — o mesmo desenho de antes. Regras puras em `dashboard-prefs.ts` (`buildRows`, `moveBlock`, `dropBlock`, `applyOrder`, `setBlockSize`, `prefsDraft`, `sameDraft`, `prefsToSave`), testadas em `src/test/lib/dashboard-blocks.test.ts`.
