...
│   │                                #   inteiro, com Aumentar-Diminuir e Remover no canto) +
│   │                                #   dashboard-prefs.ts (ordem/largura/escondidos,
│   │                                #   buildRows), quotes-notice.ts, course-capacity.ts (vagas),
...
- **Personalizar o painel** (set/2026, enxugado depois que o usuário reprovou o excesso de controles): "Personalizar" **congela o painel** no próprio `/admin/dashboard` e deixa os blocos manipuláveis — nada de janela, de amostra cortada nem de lista abstrata. O bloco continua sendo **o bloco de verdade, inteiro**, só que `inert` (não recebe clique nem foco), dentro de uma moldura tracejada (`EditableBlock`). São quatro coisas, e só: **mover** (arrastar o CARTÃO INTEIRO — `useSortable` do `@dnd-kit`; no mouse basta andar 8px, no dedo é apertar e segurar 250ms como quem move ícone de celular, e pelo teclado é Espaço, setas, Espaço, com os avisos do leitor de tela em português); **aumentar/diminuir** (UM botão que diz o que vai acontecer — "Aumentar" com meia linha, "Diminuir" com a linha toda; some abaixo de `lg`, onde tudo é largura inteira); **remover** (o bloco SOME do painel); e **adicionar de volta** (área "Blocos que não estão no painel" no fim da tela, com "+ Adicionar <nome>" por bloco; sem nenhum removido, a área não existe). Os dois botões do bloco ficam sobre a borda de cima, no canto direito, com texto e 44px — sobre a borda para não taparem número nem título. Barra sempre à vista (`EditModeBar`: no computador presa no alto depois da sidebar, no celular no pé da tela — é `fixed`, porque o `<main>` do painel tem overflow e mata o `sticky`) com o selo "Alterações não salvas", uma linha curta ("Segure e arraste um bloco para mudar a ordem."), **Restaurar padrão**, **Cancelar** e **Salvar** (só o Salvar grava). Sair da tela com o rascunho pela metade pergunta antes (`useUnsavedGuard`). Fora do modo, o painel é exatamente o de sempre.
- **Largura dos blocos**: "inteira" ocupa a linha; dois "meia" seguidos dividem a linha (no celular tudo é inteira, e "meia" sem vizinho de meia largura também ocupa a linha toda) — por isso o botão de aumentar/diminuir só aparece de `lg` para cima. Padrão de fábrica: ações, números, cotações, financeiro e agenda inteiros; cursos, cadastros incompletos e últimas ações pela metade — o mesmo desenho de antes. Regras puras em `dashboard-prefs.ts` (`buildRows`, `dropBlock`, `applyOrder`, `setBlockSize`, `toggleHidden`, `prefsDraft`, `sameDraft`, `prefsToSave`), testadas em `src/test/lib/dashboard-blocks.test.ts`.
